...
namespace sg
{
    ...
    public:
        bool        BuildProgConfigPoll(QcomJobDataPtr &job, uint8_t poll_address, uint16_t gvn, QcomProgressiveChangeData const& data);

    private:
        QcomPollPtr MakeProgConfigPoll(uint8_t poll_address, uint8_t last_control, uint16_t gvn, QcomProgressiveChangeData const& data);
